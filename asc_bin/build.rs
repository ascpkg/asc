fn main() {
    ...
    profile = format!(
        "{}{}",
        profile.to_uppercase().chars().nth(0).unwrap(),
        &profile[1..]
    );
    ...
    let library_dir =
        format!("{cargo_manifest_dir}/{library_name}/target/{library_name}/{profile}");
    let bindings_path =
        std::path::PathBuf::from(format!("{cargo_manifest_dir}/src/clang/{library_name}.rs"));
